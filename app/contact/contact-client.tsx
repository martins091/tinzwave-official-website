...
export default function ContactPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const subject = String(formData.get("subject") || "New message from website");
    const message = String(formData.get("message") || "");

    const buildMailtoFallback = () =>
      `mailto:info@tinzwave.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(
        `Name: ${name}\nEmail: ${email}\n\n${message}`
      )}`;
    ...
    if (!isEmailJsConfigured) {
      setStatus({
        type: "error",
        message: "Our online form isn't fully set up yet — click below to send this via your email app instead.",
      });
      setFallbackMailto(buildMailtoFallback());
      setSending(false);
      return;
    }
    ...
    try {
      ...
      setStatus({
        type: "error",
        message: "Failed to send message. Click below to send it via your email app instead.",
      });
      setFallbackMailto(buildMailtoFallback());
    } finally {
      setSending(false);
